...
which stars have binary companions. Unresolved and unrecognized binaries
give rise to systematic errors in planet occurrence rates, including
misclassified planets and mistakes in completeness corrections.
...
$2r_\oplus$, the net systematic error is of order 10\%.  In particular,
unrecognized binaries are unlikely to be the reason for the
apparent discrepancies between hot Jupiter occurrence rates measured in
different surveys.  For smaller planets the errors are
potentially larger: the occurrence of Earth-sized planets could be
overestimated by as much as a factor of two. We also show that whenever
...
and simply satisfying our curiosity.  One method
...
of the errors is positive or negative.  The goals of this study
were to provide a framework for dealing with these issues, and to gauge at least
the order of magnitude of the systematic errors.  In
...
other real transit survey.  We took inspiration from the
insightful analytic analyses of transit
surveys by \citet{pepper_using_2003} and \citet{pepper_searching_2005}.
...
apparent discrepancy between hot Jupiter occurrence rates measured in different
...
Imagine that a group of astronomers wants to measure the mean number of
planets per star.  They are particularly interested in planets of radius $r$ and
stars of mass $M$ and radius $R$.  They perform a
wide-field photometric search for transiting planets and detect
all the signals for which
\begin{equation}
  \frac{\delta}{\sigma}
  >
...
\label{eq:S_N_thresh}
\end{equation}
Here the signal S is the observed transit depth $\delta$, the dimensionless
fraction by which the total light fades during transits.
Note that although $\delta$ is often equated with $(r/R)^2$, this is not true when the host star
is a member of an unresolved binary.  In those cases, $\delta$ is smaller
than $(r/R)^2$ because of the constant light from the binary companion, an effect
often called the "dilution" of the transit signal.
The noise N is the fractional uncertainty in the determination of
the total flux, including all the stars that contribute to the photometric signal.
The minimum signal-to-noise ratio for detection, $({\rm S}/{\rm N})_{\rm min}$,
depends on the characteristics of the telescope and the survey.  We will assume throughout
...
i.e.}, the dependence on stellar properties is $\sigma \propto F_{\rm tot}^{-1/2} \propto (L_{\rm
tot}/d^2)^{-1/2}$, where $F_{\rm tot}$ is the total flux of the source,
$L_{\rm tot}$ is the total luminosity, and $d$ is the distance from Earth.
Whenever a star has a flux high enough to enable the detection of a planet of a certain type,
we say it is {\it searchable} for such planets.
...
tell that some of the stars are apparently overluminous.  They have detected a large number
$N_{\rm det}$ of transit signals that appear
to be produced by the desired type of planet around the desired type of star.
They have also counted the number $N_\star$ of sources in their survey that appear to
be searchable stars, i.e., stars of the desired type that are bright enough to have allowed
for the detection of a transit signal with amplitude $(r/R)^2$.
Then if $N_\star$ and $N_{\rm det}$ are
both large, the astronomers' estimate for the occurrence rate
($\Lambda$) is simply
\begin{equation}
...
the quantities in Equation~\ref{eq:occ_rate_simple} are biased:
\begin{enumerate}
%    
...
%        
    \end{itemize}
%    
    \item The number of stars that were searched, $N_\star$,
      is biased
%    
    \begin{itemize}
%        
        \item toward lower values, because it does not include all of
          the secondary stars that were inadvertently searched for transiting
          planets;
%        
        \item toward higher values, because some of the stars that
          appeared to be suitably bright to detect the
          desired type of planet are in fact binaries for which the
          amplitude of the photometric signal would have been reduced to an
          undetectable level.
%        
    \end{itemize}
%    
    \item The transit probability $p_{\rm tra}$ is biased because the
      planet-hosting star could be misclassified.  At fixed orbital period,
      the transit probability is proportional to $\rho^{-1/3}$, where
      $\rho$ is the stellar mean density \citep{Winn2010}.  Therefore, any errors in
      determining the host star's mean density
      lead to errors in the correction for the transit probability.
%    
\end{enumerate}

There are at least two other problems that may arise, which are not represented in
...
The other problem is an observational effect. Within the sample of apparently
searchable stars, 
the ratio between the number of binary and single stars will differ
from the ratio that would be found in a volume-limited sample. This is due to a type of Malmquist bias.
...
signals of a given amplitude, the binaries appear to be searchable at greater
distances from the Earth.  Whenever a star with a transiting planet
has a companion star, the light from the companion increases the total
flux and thereby improves the precision of the light curve, but also
hinders transit detection by reducing the amplitude of the transit signal.
...
$L$.  Some fraction of them exist in twin binaries.  In this scenario, stars are never misclassified
because the combined light of a binary has the same color and spectrum
as a single star.  We further assume that all planets have the same radius, $r$, and occur around single
stars and members of binaries at the same rate, $\Lambda(r)$.

Our naive observers detect transit signals with two different
amplitudes.  For the single stars, $\delta = (r/R)^2$.  For the binaries,
\begin{equation}
    \delta = \frac{L}{L_{\rm tot}}\,\left(\frac{r}{R}\right)^2  = \frac{1}{2}\left(\frac{r}{R}\right)^2.
    \label{eq:delta_obs_general} 
\end{equation}
They will interpret these lower-amplitude signals as transits
by planets of radius $r/\sqrt{2}$.

To calculate the occurrence rate of each type of planet, our observers
identify the ``stars'' from their survey that appear to have been
searchable for signals of each depth.  They reason that a signal of
amplitude $\delta$ would have been detectable for all stars out
to a certain maximum 
distance $d_0$, which
from Equation~\ref{eq:S_N_thresh} scales as
\begin{equation}
  d_0 \propto \delta \cdot L^{1/2}.
  \label{eq:dmax}
\end{equation}
They would therefore find the number of searchable stars to be
\begin{equation}
    N_\star \propto n \delta^3 L^{3/2},
\label{eq:N_searchable_prop}
\end{equation}
where $n$ is the number of stars per cubic parsec.

However, this sample of apparently searchable stars
will include some binaries.  Since the binaries are twice as
luminous as singles, at fixed $\delta$ they are included out to a
distance that is larger than $d_{\rm max}$ by a factor of $\sqrt{2}$.
But none of the stars in binaries will ever appear
to have a planet of radius $r$, because of the dilution of the transit
signal.  
The {\it apparent} occurrence rate $\Lambda\a$ of planets of radius $r$ is therefore
\begin{equation}
    \Lambda\a(r) = 
